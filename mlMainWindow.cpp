...
void mlConvertThread::run()
{
	...
	for (QString file : mFiles)
	{
		...
		QString ToolsPath = QDir::toNativeSeparators(getenv("TA_TOOLS_PATH"));
...
mlMainWindow::mlMainWindow()
{
	...
	mGamePath = QDir::toNativeSeparators(getenv("TA_GAME_PATH"));
	mGamePath.chop(1);
	mToolsPath = QDir::toNativeSeparators(getenv("TA_TOOLS_PATH"));
	mToolsPath.chop(1);
...
void mlMainWindow::BuildOutputReady(QString Output)
{
	...
	for(QString str : Output.split("\n"))
	{
		...
		else if(str.startsWith("^1") || str.toUpper().startsWith("ERROR:") || wasError || (str.length() >= 50 && str.contains("*****")))
		{
	...
	html.replace("^7", "<font color = \"light grey\">"); // default
...
void mlMainWindow::BuildFinished()
{
	...
	BuildOutputReady(QString("Errors: %1 || Warnings: %2").arg(mErrorCount).arg(mWarningCount));
	mErrorCount = 0;
	mWarningCount = 0;
}
